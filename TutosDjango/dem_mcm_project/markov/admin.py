...
class PartitionMethodForm(forms.ModelForm):
    ...
    n_cells = forms.IntegerField(
        required=False, label="Nb cellules (Voronoï)", initial=125
    )

    max_particles = forms.IntegerField(
        required=False, label="Particles max (Octree)", initial=50
    )
    max_depth = forms.IntegerField(
        required=False, label="Profondeur max (Octree)", initial=4
    )
...
class PartitionMethodAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ["name", "label", "description"]}),
        (
...
class ExperimentForm(forms.ModelForm):
    ...
    nlt = forms.IntegerField(
        label="NLT", initial=50, help_text="Nombre de paires snapshot"
    )
    step_size = forms.IntegerField(
        label="Step size", initial=1, help_text="Intervalle temporel"
    )
    start_index = forms.IntegerField(
        label="Start index", initial=0, help_text="Index du premier fichier"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "partition_method"
        ].queryset = PartitionMethod.objects.all().order_by("name", "n_cells")
...
class ExperimentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identification", {"fields": ["folder_name"]}),
        ("Partitionnement Spatial", {"fields": ["partition_method", "n_states"]}),
        (
            "Discrétisation Temporelle",
            {
...
class TransitionMatrixAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Spectre", {"fields": ["eigenvalue_2", "spectral_gap"]}),
        ("Stockage", {"fields": ["matrix_bucket_path"], "classes": ["collapse"]}),
    )
    ...
    def diag_mean_display(self, obj):
        ...
        if val > 0.7:
            ...
            color = "red"
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>', color, f"{val:.4f}"
        )
...
class RSDResultAdmin(admin.ModelAdmin):
    ...
    def rsd_initial_display(self, obj):
        try:
            val = float(str(obj.rsd_initial).replace("%", "").strip())
    ...
    def rsd_final_display(self, obj):
        try:
            val = float(str(obj.rsd_final).replace("%", "").strip())
        except (ValueError, TypeError):
            val = 0
        if val < 0.1:
            color = "green"
